...
async def get_media_content(media_id: str) -> Union[Dict[str, Any], None]:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{base_url}/media/{media_id}", headers=headers, timeout=30
        )
...
class ig_dlp(object):
    ...
    async def download_reels(self, link):
    ...
    async def here_we_download(self,all_links, typee='post'):
        filepath = []
        for media_link in all_links:
            if typee=='post':
                file = await self.download_media(media_link)  
            else:
                file=await self.download_reels(media_link)
    ...
    async def download(self):
        try:
            ...
            if mydict != "invalid_url" and mydict != "post_not_found":
                ...
                # print(download_list)
                downloaded = []
                if len(download_list) > 1 or len(photos)>=1:
                    with console.status(
                ...
                #     return False, None, []
                    ...
                    # print(videos)
                    with console.status(
                        f"[yellow]Downloading {len(videos)} Videos"
                    ) as status:
                        downloaded += await self.here_we_download(videos, typee='video')
                    print(f"Downloaded {len(downloaded)} Videos")
                # if len(photos)>=1:
                #     print(photos)
                #     with console.status(
                #         f"[yellow]Downloading {len(photos)} Items"
                #     ) as status:
                #         downloaded += await self.here_we_download(photos)
                #     print(f"Downloaded {len(downloaded)} Items✅")
                if len(downloaded) < 1:
                    return False, None, []
                # print(downloaded)
